...
# === STORE LOGS ===
log_file_path = "D:/USERS/jonas.mortelmans/PI10/logs/processing_times.csv"



#=== MAILING ===
from dotenv import load_dotenv
load_dotenv(dotenv_path="D:/USERS/jonas.mortelmans/PI10/.env")
EMAIL_SETTINGS = {
...
# === SETUP ===
CLASS_TRANSLATION_CSV = Path("Y:/plankton-imager-10/not_processed/models/2026-04-30_104022-dasysyrphus/class_name_translation.csv")
source_dir = Path("Y:/plankton-imager-10/processed/2026")
work_dir = Path("D:/USERS/jonas.mortelmans/PI10_tempUntarred")
taxon_export_root = source_dir / "by_taxon"
taxon_export_root.mkdir(parents=True, exist_ok=True)
#MAX_IMAGES_PER_TAXON_FOLDER = 100
PREVIEW_HIGH_GAP_MARGIN = 0.999
PREVIEW_HIGH_GAP_LIMIT = 150
...
quarantine_bubbles_dir = Path("Y:/plankton-imager-10/not_processed/quarantained/quarantine-bubbles")
quarantine_bubbles_dir.mkdir(parents=True, exist_ok=True)

quarantine_hitsmiss_dir = Path("Y:/plankton-imager-10/not_processed/quarantained/quarantine-hitsmisses")
quarantine_hitsmiss_dir.mkdir(parents=True, exist_ok=True)

quarantine_raisingfactor_dir = Path("Y:/plankton-imager-10/not_processed/quarantained/quarantine-raisingfactor")
quarantine_raisingfactor_dir.mkdir(parents=True, exist_ok=True)

quarantine_near_point_dir = Path("Y:/plankton-imager-10/not_processed/quarantained/quarantine-location-50m")
quarantine_near_point_dir.mkdir(parents=True, exist_ok=True)

quarantine_nogps_dir = Path("Y:/plankton-imager-10/not_processed/quarantained/quarantine-nogps")
quarantine_nogps_dir.mkdir(parents=True, exist_ok=True)

QUARANTINE_LAT = 51.235293843807796
QUARANTINE_LON = 2.9310864728604327
QUARANTINE_RADIUS_M = 50
...
TIMESTAMP = '2026-04-30_104022-dasysyrphus'
...
def get_usable_coordinates(exif_df):
    ...
    # Keep only physically valid coordinate ranges.
    coords = coords[
...
def parse_prediction_lists(entry):
    ...
    if isinstance(probs, str):
        raw_probs = [s.strip() for s in probs.split(",") if squar.strip()]
    else:
...
def extract_hitsmisses(tar_path, output_file, tar_file, status_log):
    ...
    MAX_MISS_HIT_RATIO = 100  # 75K hits -> max 750K misses (best set at 10)
...
def extract_exif_metadata(tif_paths, tar_source, batch_size=200, exiftool_path=None):
    ...
    if exiftool_path is None:
        exiftool_path = r"D:\USERS\jonas.mortelmans\PI10\exiftool-13.31_64\exiftool.exe"
...
def generate_topspecies_csv(json_path,
    ...
    for entry in data_list:
        ...
        labels = entry.get(f"top{TOP_K}_labels", [])
        probs = entry.get(f"top{TOP_K}_probs", [])
...
def check_preview_class_distribution(preview_tifs, threshold=0.4):    #more then 40% bubbles, move to quarantaine!!!
    ...
    try:
        ...
        if not hits_path.exists():
            ...
            if not ok:
                ...
                try:
                    ...
                    shutil.move(str(tar_file), str(quarantine_target))
                    status_log.append(f"Moved TAR to quarantine_hitsmiss: {quarantine_target}")
                    print(f"🚨 Quarantined {tar_file.name} → hits/misses issue")

                except Exception as mv_err:
                    status_log.append(f"⚠️ Quarantine move failed: {mv_err}")

                # Remove partial files from processing/source folder
                for suffix in REQUIRED_SUFFIXES:
                    partial_file = source_dir / f"{tar_name}{suffix}"
                    try:
                        if partial_file.exists():
        ...
        if not quarantined:
            ...
            if json_output.exists():
                ...
                should_continue, reason = check_preview_class_distribution(preview_sample_tifs, threshold=0.4)   ###more then 50% bubbles - > move to quarantaine!
            ...
            if not quarantined:
                ...
                # === Step 8b: Location quarantine check ===
                if not has_usable_coordinates(exif_df):
                    ...
                    try:
                        ...
                        status_log.append(f"Moved to quarantine-no-coordinates: {quarantine_reason}")

                        remove_partial_outputs(tar_name, status_log)

                    except Exception as mv_err:
                        status_log.append(f"⚠️ No-coordinates quarantine move failed: {mv_err}")

                elif should_quarantine_location(exif_df):
                    quarantined = True
                    quarantine_reason = "within 50 m of quarantine point"
                    ...
                    try:
                        ...
                        shutil.move(str(tar_file), str(quarantine_target))
                        print(f"🚨 Quarantined {tar_file.name}: collected within 50 m of quarantine point")
                        status_log.append(f"Moved to quarantine-location-50m: {quarantine_reason}")
                ...
                # Only continue with the expensive processing if it was NOT quarantined
                if not quarantined:
